...
class BuildWidget:
   ...
   #    cmaps.append(cmap+'_r')
    ...
    def __init__(self, *,
        ...
        for i in range(len(self.vmax)):
            try:
               ...
               max_v = 1000
            try:
               self.mag.append(stepsize or 10**(math.floor(math.log10(np.fabs(max_v))-2)))
            except AttributeError:
               self.mag.append(1)
    ...
    def _set_widgets(self):
        ...
        readout=True,
        readout_format='0.4f',
        layout=Layout(width='100%')) for i in range(n_links)]
        self.cmap_sel = [widgets.Dropdown(options=self.cmaps,
                          value=self.cmaps[0],
                          description='CMap:',
                          disabled=False,
                          layout=Layout(width='200px')) for i in range(n_links)]
    ...
    def colorbar( mappable, vmin, vmax, draw=True):
        #grid.cbar_axes[n].colorbar(mappable)
        #return
