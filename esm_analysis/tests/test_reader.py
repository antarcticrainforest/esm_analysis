...
def test_load_empty_data(mock_tmpdir, mockgrid, mockweights, esm_analysis):
    with pytest.raises(FileNotFoundError):
        with esm_analysis.RunDirectory.gen_weights(mockgrid, mock_tmpdir, prefix='test',
                model_type='DWD', infile=mockweights, overwrite=False):
            pass

def test_load_data(mock_timedir, esm_analysis):
    with esm_analysis.RunDirectory(mock_timedir, prefix='test', model_type='DWD') as run:
        ...
        assert run.name_list['picklefile'] != None
        files = Path(run.name_list['picklefile']).with_suffix('')
        run.empty_cache()
        assert run.name_list['picklefile'] == None
        assert [f for f in files.rglob('{}.*'.format(files.name))] == []


def test_gen_weights(mock_vardir, mockgrid, mockweights, esm_analysis):
    with esm_analysis.RunDirectory.gen_weights(mockgrid, mock_vardir, prefix='test',
...
def test_other_remap(mock_timedir, mockgrid, mock_tmpdir, esm_analysis):
    from pathlib import Path
    with esm_analysis.RunDirectory(Path(mock_timedir) / 'remap_grid') as run:
...
def test_grb(mock_grb_dir, esm_analysis, mockgrid, mock_tmpdir, mock_run):

    from tempfile import TemporaryDirectory
    with esm_analysis.RunDirectory(Path(mock_grb_dir), filetype='grb') as run:
        assert len(run.files) == 10
        with TemporaryDirectory() as td:
            run.remap(mockgrid, weightfile=mock_run.weightfile, out_dir=td)
            run.load_data()
            assert len(run.files) == 10


def test_reload(mock_tmpdir, mockgrid, esm_analysis):
    with esm_analysis.RunDirectory(mock_tmpdir) as run:
        assert run.run_dir == mock_tmpdir
...
def test_lookup(esm_analysis):
    MPI = esm_analysis.Reader.MPI
    CMROPH = esm_analysis.Reader.CMORPH
    echam_setup = esm_analysis.lookup('MPI')
    assert type(echam_setup) == MPI
    assert list(echam_setup.values()) == list(echam_setup.keys())

    cmorph_setup = esm_analysis.lookup('CMORPH')
    assert cmorph_setup['pr'] == 'precip'
    # Smoke test
    assert cmorph_setup['ta'] == 'ta'

    with pytest.raises(KeyError):
        esm_analysis.lookup('BALBLA')

def test_icon2datetime(esm_analysis):

    timesteps = [20091027.5, 20101027.5] # 27th October 2009/2010 12 Noon
    ts_conv = esm_analysis.icon2datetime(timesteps)
    assert (ts_conv[0].year, ts_conv[1].month, ts_conv[0].day) == (2009, 10, 27)
    assert ts_conv[1].hour == ts_conv[0].hour == 12

    ts_conv_int = esm_analysis.icon2datetime(timesteps[0])
    assert ts_conv_int == ts_conv[0]


def test_config(model_setup_with_config, model_setup_without_config, esm_analysis):
    import pandas
    conf_with = esm_analysis.Config(model_setup_with_config)
    assert type(conf_with.setup) == pandas.DataFrame

    conf_without = esm_analysis.Config(model_setup_without_config)
